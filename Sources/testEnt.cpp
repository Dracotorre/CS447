...
BOOL InitializeScene()
{
  ...
  wallTex = gTargas->add("textures/walls064.tga");
  ...
  // load the test obj
  gTestObj = new Arch1();
  gTestObj->vinit(gTargas->getPtr(wallTex), 64, 64);
  //gTestObj->vinit();
  gFloor = new Floor(20.0f, 20.0f);
  gFloor->vinit(gTargas->getPtr(marbTex), 256, 256);
